...
class PlatformFFI {
  ...
  DeskonImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libdeskon.so')
        : isLinux
            ? DynamicLibrary.open('libdeskon.so')
            : isWindows
                ? DynamicLibrary.open('libdeskon.dll')
                :
  ...
  void _startListenEvent(DeskonImpl deskonImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = deskonImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
