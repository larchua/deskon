...
/// 初始化默认内置设置
pub fn init_default_builtin_settings() {
    let mut builtin_settings = BUILTIN_SETTINGS.write().unwrap();
    
    // 初始化内置设置的默认值
    for &key in keys::KEYS_BUILDIN_SETTINGS {
        builtin_settings.insert(key.to_string(), String::new());
    }
    
    // 设置特定的默认值
    builtin_settings.insert(keys::OPTION_HIDE_SERVER_SETTINGS.to_string(), "Y".to_string());
    
    log::info!("默认内置设置已初始化，服务器设置已默认隐藏");
}
