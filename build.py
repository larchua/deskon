...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/deskon.exe')
    os.chdir('../..')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/deskon.exe')
        system2('mv target/release/deskon.exe target/release/Deskon.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\deskon.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/Deskon.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
